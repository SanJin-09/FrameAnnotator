...
## 1. 项目简介
基于 FastAPI + React 的前后端分离标注工具，支持 mp4 视频上传、抽帧、逐帧人头姿态标注（1=正, 2=上, 3=下, 4=左, 5=右, 6=左歪, 7=右歪），并一键导出裁剪图片与标签文件。当前聚焦数据采集与标注，不包含模型训练。
...
2. 上传成功后自动跳转加载页 `/loading/{session_id}`，查看抽帧进度；完成后进入 `/label/{session_id}`：逐帧点击生成固定边长裁剪框（固定 1:1，以点击点为中心）、选择标签（1=正, 2=上, 3=下, 4=左, 5=右, 6=左歪, 7=右歪）、保存并下一张。可切换“只看未标注”，使用缩略图跳转，支持导出 zip。
