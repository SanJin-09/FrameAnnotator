...
## 1. 项目简介
基于 FastAPI + React 的前后端分离标注工具，支持 mp4 视频上传、抽帧、逐帧人头姿态标注（1=上, 2=下, 3=左, 4=右, 5=左歪, 6=右歪），并一键导出裁剪图片与标签文件。当前聚焦数据采集与标注，不包含模型训练。
...
2. 上传成功后自动跳转 `/label/{session_id}`：逐帧拖拽裁剪框（固定 1:1）、选择标签、保存并下一张。可切换“只看未标注”，使用缩略图跳转。
